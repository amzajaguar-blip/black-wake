...
pattern = re.compile(r'        var newDetection = state\.detection\s+if \(boost\) newDetection = min\(1f, newDetection \+ dt \* 0\.115f\)\s+else if \(silent\) newDetection = max\(0\.0f, newDetection - dt \* \(0\.16f \+ state\.modules\.stealth \* 0\.04f\)\)\s+else newDetection = max\(0\.0f, newDetection - dt \* 0\.02f\)')
...
if pattern.search(code):
    code = pattern.sub(new_detection_logic, code)
    with open("app/src/main/java/com/blackwake/game/GameViewModel.kt", "w") as f:
        f.write(code)
    print("Regex matched and patched successfully")
else:
    print("Regex could not find the target string")
